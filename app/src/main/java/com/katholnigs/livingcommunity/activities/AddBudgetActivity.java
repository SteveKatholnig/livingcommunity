...
public class AddBudgetActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        EditText value = (EditText) findViewById(R.id.editTextAmount);
        Spinner spinner = (Spinner) findViewById(R.id.add_budget_spinner);
        EditText itemDescription = (EditText) findViewById(R.id.editTextEntryDescription);
                ...
                if (!TextUtils.isEmpty(description) && !TextUtils.isEmpty(budgetValue)){
                    ...
                    call.enqueue(new Callback<List<User>>() {
                        ...
                        public void onResponse(Call<List<User>> call, Response<List<User>> response) {
                            ...
                            for (User u : user){
                                if(u.com_id != 1){
                                    if (spinnerSelection.equals("Credit")){
                                        BudgetEntry entryToAdd = new BudgetEntry(budgetValue, ""+ 0, date, u.id, u.com_id, description);
                                        sendNetworkRequest(entryToAdd);
                                    }else{
                                        BudgetEntry entryToAdd = new BudgetEntry("" + 0, budgetValue, date, u.id, u.com_id, description);
                                        sendNetworkRequest(entryToAdd);
                                    }
                                }else{
                                    finish();
    ...
    private void sendNetworkRequest(BudgetEntry entryToAdd) {
        Retrofit.Builder builder = new Retrofit.Builder()
                .baseUrl("http://62.75.166.253/lc.app/public/")
                .addConverterFactory(GsonConverterFactory.create());
        Retrofit retrofit = builder.build();
